...
class Cliente:
    ...
    def agregar(self):
        db = get_db()
        c = db.cursor()
        c.execute('INSERT INTO clientes (nombre, correo, telefono) VALUES (?, ?, ?)',
                  (self.nombre, self.correo, self.telefono))
        db.commit()

    def actualizar(self):
        db = get_db()
        c = db.cursor()
        c.execute('UPDATE clientes SET nombre = ?, correo = ?, telefono = ? WHERE id = ?',
                  (self.nombre, self.correo, self.telefono, self.id))
        db.commit()
...
def add_cliente():
    ...
    nuevo_cliente = Cliente(None, data['nombre'], data['correo'], data['telefono'])
    nuevo_cliente.agregar()
...
def update_cliente(id):
    ...
    if cliente is None:
        return jsonify({'error': 'Cliente no encontrado'}), 404
    cliente_actualizado = Cliente(id, data['nombre'], data['correo'], data['telefono'])
    cliente_actualizado.actualizar()
